...
// Converted from TG5 Maths pt II solve1_module.js — behaviour preserved
import { QLimitRepeats, cfchk, dp, images, op, rndgen, thouSep } from '../utils.js';

var sumarrsolve = [], a, b, c, d, e, f, x, s1, type, ltr;
...
sumq = "";
    suma = "";
    sumarrsolve = QLimitRepeats(sumarrsolve, 4);   //Ensures no repeat question until at least 50% of questions shown
    var sum = sumarrsolve[sumarrsolve.length - 1];
    switch(sum) {
        case 1:
        ...
        case 2: 
            ...
            sumq += "Solve the following expression using the given values.<br>";
            sumq += "$$" + d + "xz-" + e + "xy^2$$"
        ...
        case 3:
            do {
                a = rndgen(-9, 9, 0, 1, -1);
            } while (Math.abs(a) === 1 || a === 0)
            do {
                b = rndgen(-9, 9, 0, 1, -1);
            } while (a === b || Math.abs(b) === 1 || Math.abs(a*Math.exp(b)) < 0.1);
            sumq += "Solve the following expression using the given values, rounding your answer to 2 decimal places.<br>";
            sumq += "$$ae^b$$";
            sumq += "$$where\\ a=" + a + "\\ and\\ b=" + b + "$$";
            suma += "$$\\begin{aligned}&=" + a + "e^{" + b + "}\\\\[5pt]";
            suma += "&=\\underline{\\mathbf{" + thouSep(dp(a*Math.exp(b), 2, 2), "\\ ");
            suma += "}}\\end{aligned}$$";
            break;
        case 4:
            a = rndgen(5, 18, 1, 0.1, -1);
            sumq += "Solve the following expression using the given value, rounding your answer to 2 decimal places.<br>";
            sumq += "$$\\frac{4}{3}\\pi r^3$$";
            sumq += "$$where\\ r=" + a + "$$";
            suma += "$$\\begin{aligned}&=\\frac{4}{3}\\pi\\times" + a + "^3\\\\[5pt]";
            suma += "&=\\underline{\\mathbf{" + thouSep(dp((4*Math.PI*Math.pow(a, 3))/3, 2, 2), "\\ ");
            suma += "}}\\end{aligned}$$";
            break;
    }

    notesLink = "images/20240924-TG5MathsBook2-AlgebraV1_0-APO.pdf#page=20"
    const _result = {
      question: sumq,
      solution: suma,
      notesLink: notesLink
